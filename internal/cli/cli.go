...
const orchestrationContract = "Перед началом нового шага проверь рабочую копию, историю Git и состояние обязательных проверок. Если незакоммиченные изменения явно относятся к предыдущему roadmap-шагу или шаг помечен завершённым вопреки отсутствующему обязательному коммиту либо проваленным проверкам, считай восстановление и завершение этого предыдущего шага единственной задачей текущего треда. После его исправления, полной проверки, требуемой документации и отдельного коммита заверши turn и не начинай следующий roadmap-шаг. Репозиторные команды по умолчанию запускай из канонического корня проекта. Если задаёшь более узкую рабочую директорию, передавай команде только пути относительно этой директории и не дублируй её префикс. После каждой проверки — линтера, теста, coverage gate, профайлера, анализа логов или другого обязательного gate — используй результат как обратную связь. Если проверка не прошла, диагностируй причину, исправь реализацию, тесты или конфигурацию в рамках текущего шага и повтори релевантную проверку. Не прекращай работу только из-за обычного провала проверки, если его можно исправить в текущем треде. Если текущих шагов нет, ответь отдельной строкой ORCHESTRATOR_NO_WORK. Если шаг невозможно завершить из-за внешнего блокера, который нельзя устранить в текущем треде (например, недоступное окружение, недостающие права или обязательное действие пользователя), объясни причину и ответь отдельной строкой ORCHESTRATOR_BLOCKED.\n"
